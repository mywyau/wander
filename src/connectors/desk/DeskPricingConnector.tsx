...
interface DeskPricingConnectorResult {
  success: boolean;
  message: string;
}

class DeskPricingConnector {
  async submitForm(data: UpdateDeskPricing, deskId: String): Promise<DeskPricingConnectorResult> {

    const pistachioUrl = AppConfig.basePistachioUrl();
    const apiUrl = `http://${pistachioUrl}/pistachio/desk/pricing/update/${deskId}`

    console.log("[DeskPricingConnector] submitForm called");
...
export default new DeskPricingConnector();
